...
import type { RawExhibitionDetail } from '@/shared/api/types'
// [API]
...
// [MOCK]
import * as exhibitionMock from '@/mocks/handlers/exhibitionHandlers'
...
export async function fetchCurrentExhibition(): Promise<Exhibition> {
  ...
  // [MOCK]
  const raw: RawExhibitionDetail = await exhibitionMock.getCurrentExhibition()
  return toExhibition(raw)
}
...
export async function fetchExhibitionByDate(date: IsoDate): Promise<Exhibition> {
  ...
  // [MOCK]
  const raw: RawExhibitionDetail = await exhibitionMock.getExhibitionByDate(date)
  return toExhibition(raw)
}
...
export async function fetchArchive(params: {
  ...
  // [MOCK]
  const result = await exhibitionMock.getArchive({ limit, cursor: params.cursor })
  return {
    items: result.data.exhibitions.map(toExhibitionSummary),
 ...
 * 실패해도 재시도 1회 후 조용히 포기한다(프런트 §9.3).
 */
export async function logExhibitionView(date: IsoDate): Promise<void> {
  try {
    ...
    // [MOCK]
    await exhibitionMock.logExhibitionView(date)
  } catch {
