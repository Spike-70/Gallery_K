...
**화면 구성** — 전화번호(ID) 입력 · 비밀번호 입력 · `입장` 버튼 · `비밀번호를 잊으셨나요?` 링크 · **구분선 + 소셜 로그인 버튼(카카오·구글)** · `첫 화면으로`
...
- **소셜 로그인은 리다이렉트 방식이다.** 팝업을 쓰지 않는다 — iOS Safari에서 차단되기 쉽고, 차단되면 아무 일도 일어나지 않아 사용자가 원인을 알 수 없다. 화면이 실제로 넘어가야 "지금 무슨 일이 일어나는지"가 보인다.
- 소셜 버튼은 **평범한 링크**다. 자바스크립트가 로드되기 전에 눌러도 동작한다 — 반응 없는 버튼을 두 번 세 번 누르게 두지 않는다.
- 연결된 계정이 없는 소셜로 처음 들어오면 **A-4. 계정 연결**로 보낸다. 소셜만으로는 계정이 만들어지지 않는다 (§6.2-1).
...
| 소셜 동의 화면에서 취소 | 조용히 A-1으로 돌아온다. 오류 문구를 띄우지 않는다 — 사용자가 스스로 그만둔 것이다 |
| 소셜 연결 실패 | 화면 상단 배너로 한국어 안내. 전화번호 로그인은 그대로 쓸 수 있다 |

**나가는 길** — C. 갤러리 / A-2. 비밀번호 재설정 / A-4. 계정 연결 / A. 첫 화면

---

### 6.2-1 A-4. 계정 연결 `[신규]`

**목적** — 소셜 계정을 이 서비스의 회원과 이어 붙인다.
**진입 경로** — A-1에서 소셜로 들어왔는데 연결된 계정이 없을 때. **직접 주소를 쳐서 올 수 없다** — 유효한 연결 티켓이 없으면 A-1으로 되돌린다.

**왜 자동으로 가입시키지 않는가** — 아침 알림 · 큐레이터 대행 가입 · 차단 · 회원 관리가 전부 **전화번호**에 걸려 있다. 전화번호 없는 회원이 생기면 운영 화면에서 그 사람을 다룰 수 없다. 소셜은 로그인 수단이지 신원이 아니다.

**화면 구성** — 어느 제공자로 들어왔는지 안내 · 전화번호 입력 · (기본 모드) 비밀번호 입력 · `연결하기` · 하단 전환 링크

| 모드 | 입력 | 동작 |
|---|---|---|
| **이미 회원이신가요?** (기본) | 전화번호 + 비밀번호 | 소유를 확인하고 연결한다. 비밀번호를 요구하는 이유는 남의 번호를 적어 계정을 가져가지 못하게 하기 위해서다 |
| **처음이신가요?** | 전화번호 + 이름 + 약관 동의 | 새 계정을 만들고 연결한다. 비밀번호는 만들지 않는다 |

**기능 상세**
- 두 모드는 사용자가 **직접 고른다.** 전화번호를 넣었을 때 서버가 "이미 가입된 번호입니다"를 먼저 알려주면 그 자체로 회원 명단 조회 수단이 된다.
- 신규 가입 모드는 **가입 잠금(§6.11)을 그대로 따른다.** 소셜이 잠금의 뒷문이 되면 폐쇄형 서비스라는 전제가 깨진다.
- 연결 유효 시간은 **10분**이다. 지나면 처음부터 다시 한다.

**상태와 예외**

| 상태 | 처리 |
|---|---|
| 비밀번호 불일치 · 미가입 번호 · 차단 회원 | 모두 `전화번호 또는 비밀번호가 맞지 않습니다` |
| 신규 모드인데 이미 가입된 번호 | `이미 가입된 번호입니다` + `이미 회원이신가요?`로 전환 안내 |
| 가입 잠금 중 | `지금은 새로운 회원을 받고 있지 않습니다` |
| 이미 다른 계정에 연결된 소셜 | `이미 다른 계정에 연결된 소셜 계정입니다` — 어느 계정인지는 알려주지 않는다 |
| 10분 경과 | `연결 시간이 지났습니다` + A-1으로 |
