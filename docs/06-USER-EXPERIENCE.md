...
**레이아웃** — 화면 제목 `입장` · 전화번호 필드 · 비밀번호 필드 · `입장` 버튼 · `비밀번호를 잊으셨나요?` · **구분선 `또는`** · **소셜 버튼(카카오·구글)** · `첫 화면으로`
...
| 소셜 버튼 | **`<a href>` 링크다.** `onClick`으로 주소를 바꾸지 않는다 — JS 로드 전 클릭이 무반응이면 사용자는 같은 자리를 반복해 누른다 |
| 소셜 이동 | **리다이렉트.** 팝업을 열지 않는다. 화면이 실제로 넘어가야 무슨 일이 일어나는지 보인다 |
| 소셜 버튼 노출 | 서버가 켜졌다고 알려준 제공자만. 목록이 비면 구분선과 영역 전체를 그리지 않는다 |
| 소셜 최소 높이 | 전화번호 `입장` 버튼과 같은 `touch` 높이. 소셜이 부차적으로 보이지 않게 한다 |
...
| 소셜 동의 취소 | **표시 없음** | 사용자가 스스로 그만둔 것이다. 실패로 알리지 않는다 |
| 소셜 실패(`?social_error=`) | 폼 상단 배너 | 오류 코드를 §5.2의 한국어 문구로 번역해 그대로. 전화번호 로그인은 계속 쓸 수 있다 |

### 3.2-1 A-4. 계정 연결

소셜로 처음 들어온 사람이 자기 계정과 이어 붙이는 화면. **직접 주소로 올 수 없다** — 연결 티켓이 없으면 A-1으로 되돌린다.

**레이아웃** — 화면 제목 `계정 연결` · `카카오 계정으로 들어오셨습니다` 안내 · 전화번호 필드 · (모드별 추가 필드) · `연결하기` 버튼 · 모드 전환 링크

**두 모드**

| 모드 | 추가 필드 | 전환 링크 문구 |
|---|---|---|
| 이미 회원 (기본) | 비밀번호 | `처음이신가요? 새로 시작하기` |
| 처음 | 이름 · 약관 동의 체크 | `이미 회원이신가요? 기존 계정에 연결하기` |

**상호작용**

| 항목 | 사양 |
|---|---|
| 모드 전환 | 같은 화면 안에서 바꾼다. 전화번호 입력값은 유지한다 — 다시 치게 하지 않는다 |
| 모드 선택 주체 | **사용자.** 서버가 "이미 가입된 번호입니다"를 먼저 알려주면 그게 곧 회원 명단 조회 수단이다 |
| 제출 | 필수 필드가 채워지면 활성 |
| 성공 | C 갤러리로 직행 |
...
| 상황 | 표시 위치 | 문구 |
|---|---|---|
| 연결 모드 인증 실패 | 폼 상단 배너 | `전화번호 또는 비밀번호가 맞지 않습니다.` |
| 신규 모드인데 가입된 번호 | 폼 상단 배너 | `이미 가입된 번호입니다.` + 연결 모드 전환 링크 |
| 가입 잠금 | 폼 상단 배너 | `지금은 새로운 회원을 받고 있지 않습니다.` |
| 이미 연결된 소셜 | 폼 상단 배너 | `이미 다른 계정에 연결된 소셜 계정입니다.` |
| 10분 경과 | A-1으로 이동 | `연결 시간이 지났습니다. 처음부터 다시 시도해 주세요.` |
...
**레이아웃** — 화면 제목 `설정` · 알림 섹션 · 화면 섹션 · **연결된 로그인 섹션** · 계정 섹션 · `갤러리 화면으로`
...
| 연결된 로그인 | 연결된 제공자 | 이름 + 연결일. 우측에 `해제` 텍스트 링크 |
| 연결된 로그인 | 미연결 제공자 | `연결하기` — A-1과 같은 `<a href>` 링크. 돌아올 곳은 설정 화면이다 |
| 계정 | `이름`·`전화번호` | 읽기 전용 표시 |
| 계정 | `비밀번호 변경` | TextLink. **`has_password=false`면 항목 자체를 감춘다** — 없는 비밀번호를 바꾸라고 하지 않는다 |
...
| 상황 | 표시 |
|---|---|
| 비밀번호 없고 연결이 하나뿐 | `해제` 비활성 + `마지막 로그인 수단은 해제할 수 없습니다.` — 해제하면 들어올 길이 없어진다 |
| 해제 성공 | 목록에서 사라지고 토스트 `연결을 해제했습니다.` |
