...
**화면과 서버가 대화하는 창구**입니다. 식당의 주문서를 떠올리면 됩니다. 손님이 "김치찌개 하나요"라고 말하면 주방이 알아듣고 김치찌개를 내옵니다. 이때 "김치찌개 하나요"라는 **말의 형식**과 "이렇게 담아서 나간다"는 **답의 형식**을 미리 정해놓은 것이 API 명세서입니다.

**왜 이렇게 길고 자세한가요?**

화면 만드는 사람과 서버 만드는 사람이 **다른 사람이거나, 다른 시점에 작업하기 때문**입니다. 두 사람이 "전화번호를 `phone`으로 보낼까 `phoneNumber`로 보낼까"를 매번 상의하면 일이 안 됩니다. 미리 다 정해두면 각자 자기 쪽을 만들고 나중에 붙여도 맞습니다.

**이 문서에서 가장 중요한 두 곳**

1. **§2.2 표준 응답 봉투** — 서버의 모든 답이 똑같은 겉포장을 쓴다는 약속. 이것 하나로 화면 코드가 훨씬 단순해집니다.
2. **§5 오류 코드 카탈로그** — 문제가 생겼을 때 서버가 뭐라고 답하고, 화면이 사용자에게 뭐라고 보여줄지의 목록.

**읽는 요령**

- **§4 엔드포인트 총람**이 목차입니다. 창구가 50개 있고, 각각 뭘 하는지 한 줄씩 나옵니다.
- §6~§10은 그 50개를 하나씩 자세히 설명합니다. **처음부터 읽지 말고 필요한 것만 찾아보세요.**
- `GET`은 "가져오기", `POST`는 "새로 만들기·실행하기", `PUT`은 "통째로 바꾸기", `PATCH`는 "일부만 바꾸기", `DELETE`는 "지우기"입니다.
- 모르는 단어는 [용어 사전](00-용어사전.md)에서. 원문은 `../02-API-SPEC.md`입니다.
...
> **📘 쉬운 설명**
>
> API를 설계할 때의 7가지 기준입니다. 이 중 **AP-3(화면 단위로 응답을 설계한다)**이 다소 이례적인 선택입니다.
>
> 보통은 "회원 정보 창구", "전시 정보 창구"처럼 **정보 종류별로** 창구를 나눕니다. 그런데 그러면 첫 화면 하나를 그리려고 창구 4개를 찾아다녀야 하고, 그만큼 느려집니다.
>
> 이 서비스는 "앱을 열고 3번 안에 그림을 본다"가 목표라서, **화면 하나에 창구 하나**를 원칙으로 삼았습니다. 이론적으로 덜 예쁘지만 사용자에게는 더 빠릅니다.
...
> **📘 쉬운 설명**
>
> 개별 창구를 설명하기 전에, **모든 창구에 공통으로 적용되는 약속**을 먼저 정합니다.
>
> 여기서 정하는 것들: 주소를 어떻게 짓는지, 답을 어떤 모양으로 주는지, 목록을 어떻게 나눠 주는지, 로그인은 어떻게 확인하는지. 이걸 창구마다 따로 정하면 50개가 다 달라집니다.

### 2.1 엔드포인트 규약

> **📘 쉬운 설명**
>
> 창구 주소를 짓는 규칙입니다. 주목할 결정 하나 — **전시는 날짜로 식별합니다.**
>
> 보통 프로그램은 모든 것에 고유번호(`a3f9c...`)를 붙여 쓰는데, 여기서는 전시만큼은 `2026-08-30`처럼 날짜를 씁니다. 큐레이터에게 전시의 이름은 곧 날짜이고, 주소만 봐도 무엇에 대한 요청인지 알 수 있어 **운영 중 실수가 줄어들기** 때문입니다.
>
> "값이 없으면 `null`을 넣고 키를 생략하지 않는다"는 규칙도 실무적으로 중요합니다. 어떤 때는 있고 어떤 때는 없는 항목이 있으면, 화면 코드가 매번 "있나 없나"를 확인해야 해서 지저분해집니다.
...
> **📘 쉬운 설명 — 이 문서에서 가장 중요한 절**
>
> **봉투(Envelope)**란, 서버가 무슨 답을 하든 **똑같은 겉포장에 담아 보내는 것**입니다.
>
> 성공하든 실패하든 답의 겉모습은 항상 이렇습니다.
>
> - `success` — 성공했나요? (예/아니오)
> - `data` — 성공했다면 그 내용
> - `error` — 실패했다면 그 이유
> - `meta` — 요청 번호, 서버 시각 등 부가 정보
>
> **왜 이게 그렇게 중요한가요?** 봉투가 없으면 화면 코드는 창구마다 "이 창구는 실패하면 어떻게 답하더라?"를 따로 외워야 합니다. 창구가 50개면 50가지 처리 방식이 생기고, 그중 몇 개는 반드시 빠뜨립니다. 그러면 **사용자는 아무 반응 없는 화면을 보게 됩니다.**
>
> 봉투가 있으면 화면은 `success` 하나만 보고 갈라지면 됩니다. **오류 처리 코드가 서비스 전체에 딱 하나만 존재합니다.**
...
> **📘 쉬운 설명**
>
> 모든 답에 딸려오는 부가 정보입니다. 두 가지가 특히 유용합니다.
>
> - `request_id` — 요청마다 붙는 고유 번호입니다. 사용자가 "화면이 이상해요"라고 하면 이 번호로 서버 기록에서 그 요청만 정확히 찾아낼 수 있습니다. 번호가 없으면 수만 줄의 기록을 뒤져야 합니다.
> - `server_date` — **서버가 알려주는 오늘 날짜**입니다. 사용자 휴대폰의 시계가 틀어져 있을 수 있으므로, 날짜는 언제나 서버 것을 믿습니다.
...
> **📘 쉬운 설명**
>
> **페이지네이션**은 목록을 한 번에 다 주지 않고 나눠 주는 것입니다. 검색 결과 아래의 "1 2 3 4 ..." 같은 것이죠.
>
> 여기서는 두 가지 방식을 씁니다.
>
> - **커서 방식** — "여기 다음부터 주세요"라고 북마크를 넘기는 방식. 아래로 계속 스크롤하는 화면에 적합합니다
> - **페이지 방식** — "3페이지 주세요"라고 번호로 요청. 전체 개수를 알아야 하는 관리 화면에 적합합니다
>
> **왜 커서 방식이 따로 필요한가요?** 목록을 보는 동안 새 항목이 추가되면, 페이지 번호 방식에서는 항목이 한 칸씩 밀려서 **2페이지에서 이미 본 항목이 또 나오거나 어떤 항목은 건너뛰어집니다.** 커서 방식은 이 문제가 없습니다.
>
> 마지막 줄이 중요합니다 — 항목이 30개뿐인 목록도 **똑같은 구조로** 답합니다. 예외를 만들지 않아야 화면 코드가 하나로 유지됩니다.
...
> **📘 쉬운 설명**
>
> 실패했을 때 서버가 돌려주는 내용의 구조입니다. 눈여겨볼 점 세 가지.
>
> **① `code`와 `message`가 따로 있습니다.**
> `code`는 컴퓨터가 읽는 이름(`AUTH_INVALID_CREDENTIALS`)이고, `message`는 사람이 읽는 문장("전화번호 또는 비밀번호가 맞지 않습니다")입니다. 나누는 이유는, 문구를 다듬고 싶어질 때 **화면 코드를 건드리지 않고 문장만 바꿀 수 있게** 하기 위해서입니다.
>
> **② `field_errors`가 따로 있습니다.**
> "비밀번호가 짧습니다" 같은 오류는 화면 위쪽에 띄우는 것보다 **그 입력칸 바로 아래에** 띄우는 게 훨씬 친절합니다. 어느 칸의 문제인지를 서버가 알려주면 화면이 그 자리에 붙일 수 있습니다.
>
> **③ `retryable`이 있습니다.**
> "다시 해보면 될 수도 있는 실패"(잠깐의 통신 장애)와 "다시 해도 소용없는 실패"(비밀번호 틀림)를 구분합니다. 앞의 것만 자동으로 재시도합니다.
...
> **📘 쉬운 설명**
>
> 목록을 걸러내고 순서를 정하는 방법입니다. `sort=-created_at`에서 앞의 `-`는 "거꾸로"(최신순)를 뜻합니다.
>
> 마지막 줄이 중요한 결정입니다 — **모르는 조건이 들어오면 무시하지 않고 거부합니다.**
>
> 흔한 사고가 있습니다. 개발자가 `status`를 `staus`로 오타 냈는데 서버가 조용히 무시하면, 화면에는 **걸러지지 않은 전체 목록이 아무 경고 없이** 표시됩니다. 관리 화면에서는 이게 심각한 문제가 될 수 있어서, 차라리 대놓고 실패하게 만듭니다.
...
> **📘 쉬운 설명**
>
> "로그인한 상태를 어떻게 기억하는가"입니다.
>
> 방식은 이렇습니다. 로그인에 성공하면 서버가 **위조하기 어렵게 서명된 쪽지(JWT)**를 만들어 브라우저에 쿠키로 넣어줍니다. 그다음부터는 요청할 때마다 브라우저가 그 쪽지를 자동으로 함께 보내고, 서버는 서명을 확인해 "아 이 사람이구나" 합니다.
>
> **`HttpOnly`가 핵심 보안 장치입니다.** 이 표시가 붙은 쿠키는 **화면의 자바스크립트 코드가 읽을 수 없습니다.** 그래서 혹시 악성 코드가 화면에 끼어들어도 로그인 쪽지를 훔쳐갈 수 없습니다. 그래서 이 서비스는 토큰을 응답 내용에 담아 보내지 않습니다.
>
> **90일 자동 로그인**은 기획서의 결정입니다(GAP-14). 시니어 사용자가 매번 로그인해야 한다면 그 자체가 이탈 이유가 되기 때문에, 체크박스로 고르게 하지 않고 기본 동작으로 만들었습니다.
>
> **`token_version`** — 회원 정보에 있는 숫자 하나와 쪽지 안의 숫자를 대조합니다. 비밀번호를 바꾸면 이 숫자가 올라가고, 예전 쪽지들은 한꺼번에 무효가 됩니다.
...
> **📘 쉬운 설명**
>
> 응답에 붙는 3자리 숫자입니다. 대략 이렇게 나뉩니다.
>
> - `2xx` — 잘됨
> - `4xx` — **요청한 쪽 잘못** (없는 주소, 로그인 안 함, 형식 오류)
> - `5xx` — **서버 잘못** (고장)
>
> `404`(없음)를 웹 서핑하다 본 적 있을 겁니다. 여기서 눈에 띄는 건 **`204`를 안 쓰기로 한 것**입니다. `204`는 "성공했고 돌려줄 내용은 없음"인데, 그러면 봉투(§2.2)가 안 옵니다. 봉투 규칙에 예외를 만들지 않으려고 일부러 안 씁니다.
...
> **📘 쉬운 설명**
>
> **캐시**는 한 번 받은 것을 잠시 저장해두고 다시 쓰는 것입니다. 매번 새로 받으면 느리고 데이터 요금도 나갑니다.
>
> `ETag`는 이런 대화를 가능하게 합니다.
> - 화면: "전시 정보 주세요. 저는 3번 버전을 갖고 있어요"
> - 서버: "여전히 3번입니다" (내용은 안 보냄)
>
> 이러면 통신량이 거의 0이 됩니다. 과거 전시는 절대 안 바뀌므로 이 방식이 특히 잘 맞습니다.
>
> **관리자 화면은 캐시를 아예 안 씁니다.** 운영자가 방금 바꾼 내용이 화면에 안 보이면 혼란스럽기 때문입니다.
...
> **📘 쉬운 설명**
>
> "짧은 시간에 너무 많이 요청하면 막는다"는 규칙입니다.
>
> 가장 중요한 건 첫 줄, **로그인 5회 실패 시 10분 잠금**입니다. 이게 없으면 누군가 프로그램을 돌려서 비밀번호를 1초에 수천 개씩 시도할 수 있습니다. 사람은 5번을 틀릴 수 있어도 그 이상 빠르게 시도하진 않습니다.
...
> **📘 쉬운 설명**
>
> 브라우저에는 "다른 주소의 서버에 함부로 요청하지 마라"는 안전장치가 있습니다. 이걸 풀어주는 절차가 CORS인데, 설정이 까다롭고 사고가 잦습니다.
>
> 이 서비스는 **화면과 서버를 같은 주소로 배포**해서 이 문제를 아예 안 만들었습니다. 문제를 푸는 대신 문제가 생기지 않는 구조를 고른 것입니다.
...
> **📘 쉬운 설명**
>
> **멱등하다**는 것은 "같은 요청을 여러 번 보내도 결과가 한 번 보낸 것과 같다"는 뜻입니다.
>
> 왜 중요하냐면 — 지하철에서 통신이 끊겼다 붙었다 할 때, 화면은 요청이 갔는지 안 갔는지 모른 채 다시 보낼 수 있습니다. 이때 "입장 기록"이 두 번 쌓이면 통계가 틀어집니다.
>
> 그래서 기록 창구들은 **두 번 들어와도 한 번으로 처리**되게 만들었습니다. 별도의 복잡한 장치 없이 창구 설계 자체로 해결한 것입니다.
...
> **📘 쉬운 설명**
>
> 여러 창구에서 반복해서 쓰이는 **정보의 모양**을 미리 정의해둔 곳입니다.
>
> 예를 들어 "그림 정보"는 갤러리 화면에도, 아카이브에도, 관리자 화면에도 나옵니다. 매번 따로 정의하면 화면마다 조금씩 달라지고, 그러면 화면 코드도 각각 만들어야 합니다. **한 번 정의하고 재사용**합니다.
...
> **📘 쉬운 설명**
>
> 그림 한 점에 이미지가 **세 가지 크기**로 준비됩니다.
>
> | 종류 | 크기 | 쓰이는 곳 | 대략 용량 |
> |---|---|---|---|
> | `thumb` | 400×400 | 12개 격자 화면 | 약 35KB |
> | `display` | 긴 변 1600px | 그림 한 점 보는 화면 | 약 180KB |
> | `origin` | 원본 그대로 | 손가락으로 확대할 때만 | 수 MB |
>
> 왜 나눴을까요? 원본 12장을 그대로 내려보내면 **한 번 볼 때마다 수십 MB**가 나갑니다. 매일 아침 보는 서비스에서 그건 데이터 요금 부담이자 이탈 이유입니다.
>
> `lqip`는 아주 작고 흐릿한 미리보기입니다. 진짜 이미지가 오기 전 뿌옇게 깔아두면 화면이 텅 비어 보이지 않습니다.
>
> `aspect_ratio`(가로세로 비율)를 함께 주는 이유 — 이미지가 도착하기 전에 **자리를 미리 잡아두기 위해서**입니다. 이게 없으면 이미지가 하나씩 뜰 때마다 화면이 아래로 덜컥덜컥 밀립니다.
...
> **📘 쉬운 설명**
>
> 갤러리 화면 하나를 그리는 데 필요한 정보 전부입니다.
>
> 여기서 이 서비스만의 특징이 드러납니다. **`exhibition_date`(발행일)와 `viewing_date`(관람일)가 따로 있습니다.**
>
> 큐레이터가 오늘 전시를 못 올렸으면 어제 전시가 그대로 걸립니다. 그러면 8월 31일에 접속한 사람이 8월 30일 전시를 보게 되죠. 화면에는 오늘 날짜(8월 31일)를 그대로 두고, 제목 아래에 작게 "8월 30일의 전시"라고 적습니다.
>
> **이 문구를 서버가 완성해서 줍니다.** 화면이 날짜를 비교해 직접 만들지 않는 이유는, 사용자 휴대폰 시계가 틀어져 있으면 이상한 문구가 나오기 때문입니다.
...
> **📘 쉬운 설명**
>
> 로그인한 사람의 정보입니다. `phone_masked`를 보세요 — **관람자 화면에는 전화번호 전체를 절대 보내지 않습니다.** `010-****-5678`처럼 가려서 보냅니다.
>
> 화면에 안 보이면 된다고 생각하기 쉽지만, 브라우저 개발자 도구를 열면 서버가 보낸 내용이 그대로 보입니다. **화면에 안 그리는 것과 보내지 않는 것은 다릅니다.**
...
> **📘 쉬운 설명**
>
> 관리자 화면의 회원 목록 한 줄입니다. 여기서만 **전화번호 전체**를 보냅니다. 큐레이터가 "알림을 못 받고 있는 분에게 전화해서 도와주는" 화면이라 번호가 필요하기 때문입니다.
>
> `push_status`와 `push_platforms`가 재미있는 항목입니다. iPhone 사용자는 홈 화면에 추가하지 않으면 알림을 받을 수 없는데, 본인은 그걸 모릅니다. 목록에 `iOS` + `알림 없음`으로 표시되면 **큐레이터가 전화 한 통으로 도와줄 수 있습니다.**
...
> **📘 쉬운 설명**
>
> 관리자 화면의 날짜 목록 한 줄입니다. 마지막 문단이 중요한 설계 판단입니다.
>
> `edit_mode`(이 날짜에 무엇을 할 수 있는가)를 **서버가 정해서 알려줍니다.** 화면이 "이 날짜는 과거니까 수정만 되겠네" 하고 직접 판단하지 않습니다.
>
> 왜냐하면 "지난 날짜에는 새 전시를 못 건다", "쓰다 만 원고가 있으면 오늘로 옮길 수 있다" 같은 규칙은 **업무 규칙**인데, 이걸 화면과 서버 양쪽에 적어두면 나중에 규칙이 바뀔 때 한 쪽만 고치고 다른 쪽을 잊게 됩니다.
...
> **📘 쉬운 설명 — 이 문서의 목차**
>
> 창구 50개의 전체 목록입니다. **여기서 필요한 것을 찾아 §6~§10의 해당 절로 가면 됩니다.**
>
> `권한` 열을 보세요.
> - `PUBLIC` — 로그인 안 해도 됨
> - `MEMBER` — 로그인한 회원만
> - `CURATOR` — 큐레이터(관리자)만
>
> `릴리스` 열의 `v1.1`은 **지금은 안 만들지만 나중에 만들 것**입니다. 지금 만들지 않아도 미리 정해두는 이유는, 나중에 붙일 때 "이거 어떻게 할까요?"를 다시 논의하지 않기 위해서입니다.
...
> **📘 쉬운 설명**
>
> 일이 잘못됐을 때 서버가 뭐라고 답하는지의 **전체 목록**입니다.
>
> 왜 목록으로 관리하냐면, 이런 게 없으면 개발자마다 그때그때 이름을 지어냅니다. `LOGIN_FAIL`, `AUTH_FAILED`, `INVALID_LOGIN`이 뒤섞이고, 화면 쪽은 어느 것에 대응해야 할지 모르게 됩니다.
>
> 각 코드에 **사용자에게 그대로 보여줄 수 있는 한국어 문장**이 붙어 있는 것도 중요합니다. 개발자가 급할 때 "오류가 발생했습니다(code: 500)" 같은 문구를 만들어내지 않도록, 미리 다 써두었습니다.
...
> **📘 쉬운 설명**
>
> 표 아래 굵은 문장이 보안의 핵심입니다. **가입 안 된 번호, 비밀번호 틀림, 차단된 회원 — 이 셋에 전부 똑같이 답합니다.**
>
> 왜 그럴까요? 만약 "가입되지 않은 번호입니다"라고 친절하게 답하면, 공격자는 전화번호를 하나씩 넣어보며 **어떤 번호가 이 서비스의 회원인지 알아낼 수 있습니다.** 30~100명의 지인만 쓰는 폐쇄형 서비스에서는 그 자체가 개인정보 유출입니다.
>
> 불친절해 보이지만 의도된 불친절입니다.
...
> **📘 쉬운 설명**
>
> 여기서부터 창구를 하나씩 자세히 봅니다. 이 장은 **로그인 전에 쓰는 창구들**입니다.
>
> 각 창구는 이런 형식으로 설명됩니다.
> - **권한** — 누가 쓸 수 있나
> - **요청** — 무엇을 보내야 하나
...
> **📘 쉬운 설명**
>
> 첫 화면에 필요한 모든 것을 **한 번에** 주는 창구입니다. 오늘 날짜, 전시 제목, 휴관 공지, 가입 가능 여부, 관리자 여부까지.
>
> 마지막 줄이 이 창구의 진짜 특징입니다 — **"이 엔드포인트는 실패하지 않는다."**
>
> 데이터베이스가 죽어도 날짜와 버튼만 채워서 답합니다. 기획서에 "장애 시에도 첫 화면은 항상 뜬다"는 요구가 있기 때문입니다. 사용자 입장에서, 서비스가 아예 안 열리는 것과 "오늘의 전시 제목만 안 보이는 것"은 완전히 다른 경험입니다.
...
> **📘 쉬운 설명**
>
> 가입 창구입니다. 받는 정보가 **전화번호·비밀번호·이름 딱 3개**뿐입니다. 이메일도, 생년월일도, 주소도 받지 않습니다.
>
> 이유가 두 가지입니다. ① 입력 항목이 많을수록 가입 도중 포기하는 사람이 늘어납니다. ② **갖고 있지 않은 개인정보는 유출될 수 없습니다.**
>
> 가입에 성공하면 곧바로 로그인 상태가 되어 갤러리로 직행합니다. "가입 완료! 이제 로그인하세요"라는 화면을 한 번 더 보여주지 않습니다.
...
> **📘 쉬운 설명**
>
> 로그인 창구입니다. 성공하면 서버가 로그인 쪽지(쿠키)를 심어줍니다. 그다음부터는 화면이 아무것도 하지 않아도 브라우저가 알아서 그 쪽지를 붙여 보냅니다.
>
> 실패하면 "몇 번 틀렸는지"를 세는 기록이 하나 올라가고, 성공하면 그 기록이 지워집니다(§2.10).
...
> **📘 쉬운 설명**
>
> 앱을 켤 때 가장 먼저 부르는 창구입니다. **"저 로그인되어 있나요?"**를 묻는 것이죠.
>
> 브라우저에 쿠키가 있어도 그게 아직 유효한지(만료되지 않았는지, 비밀번호가 바뀌지 않았는지)는 서버만 압니다. 그래서 화면을 그리기 전에 한 번 물어봅니다.
>
> 이때 **이미지를 볼 수 있는 열쇠(미디어 쿠키)도 함께 갱신**해 줍니다. 두 번 왕복하지 않으려는 것입니다.
...
> **📘 쉬운 설명**
>
> 그림 이미지를 **아무나 볼 수 없게** 만드는 장치입니다.
>
> 이미지 파일은 비공개 창고(S3)에 있고, 주소를 안다고 열리지 않습니다. 열려면 "이 사람이, 이 시간까지, 이 폴더를" 볼 수 있다는 **임시 열쇠**가 필요합니다.
>
> 열쇠를 주는 방법이 두 가지 있는데,
> - **이미지마다 열쇠를 붙인 주소를 만든다** — 그림 12장 × 3종 = 36개를 매번 만들어야 하고, 주소가 계속 바뀌어 캐시가 안 듣습니다
> - **폴더 전체에 대한 열쇠를 쿠키로 한 번 준다** ← **이 방식을 골랐습니다**
>
> 뒤쪽이 훨씬 효율적입니다. 다만 쿠키가 만료되면 **모든 이미지가 한꺼번에 안 보이게** 되므로, 만료 전에 갱신하는 책임을 화면 쪽에 명확히 지정해 두었습니다.
...
> **📘 쉬운 설명**
>
> 로그인한 관람자가 쓰는 창구들입니다. 전시 보기, 그림 보기, 지난 전시 목록, 그리고 기록 남기기.

### 7.1 `GET /exhibitions/current` — 현재 전시 (C 화면)

> **📘 쉬운 설명 — 이 서비스에서 가장 자주 불리는 창구**
>
> "지금 걸려 있는 전시 주세요"입니다. 핵심은 **"오늘의 전시"가 아니라 "가장 최근에 걸린 전시"**를 준다는 점입니다.
>
> 큐레이터가 오늘 못 올렸으면 어제 것을, 어제도 안 올렸으면 그저께 것을 줍니다. **관람자에게 빈 날은 존재하지 않습니다.** 그래서 "오늘 전시가 없습니다"라는 오류가 아예 없습니다(서비스 시작 전은 예외).
>
> 마지막 주의사항도 중요합니다 — **이 창구는 입장 기록을 남기지 않습니다.** 기록은 §7.4의 별도 창구가 담당합니다. 조회와 기록을 섞으면 오프라인 캐시나 미리 불러오기 같은 기능을 쓸 수 없게 됩니다.
...
> **📘 쉬운 설명**
>
> "지난 전시" 목록입니다. **"지난 30일"이 아니라 "최근 30개 전시"**인 점이 포인트입니다.
>
> 큐레이터가 일주일에 세 번만 올리는 시기라면, "지난 30일"로 세면 목록에 13개밖에 안 나옵니다. "최근 30개"로 세면 언제나 30개가 채워집니다. 발행이 뜸해도 관람자가 볼 게 줄어들지 않게 하는 배려입니다.
...
> **📘 쉬운 설명**
>
> "누가 오늘 갤러리에 왔다"를 남기는 창구입니다. 사용자는 이런 게 오가는 줄 모릅니다.
>
> **하루에 몇 번을 눌러도 하루 1회로만 셉니다.** 이 서비스가 알고 싶은 건 "얼마나 자주 열었나"가 아니라 "며칠이나 왔나"이기 때문입니다.
>
> 날짜 판정도 **서버가 합니다.** 사용자 휴대폰 시계를 믿지 않습니다.
...
> **📘 쉬운 설명**
>
> "이 그림을 봤다"를 남깁니다. 여기서 세심한 규칙 하나 — **화면에 1.5초 이상 머문 경우에만 보냅니다.**
>
> 손가락으로 휙휙 넘기며 지나친 그림까지 "봤다"고 세면, "하루에 평균 몇 점을 보시나요?"라는 통계가 의미 없어집니다. **측정하려는 것을 실제로 측정하기 위한 조건**입니다.
...
> **📘 쉬운 설명**
>
> 회원이 자기 계정을 다루는 창구들입니다. 설정 화면(C-4)에서 하는 일들이 여기에 대응합니다.
...
> **📘 쉬운 설명**
>
> 탈퇴 창구입니다. 요청에 `confirm: true`를 **반드시 넣게** 만든 이유는, 실수로 이 창구가 불려서 계정이 사라지는 일을 한 겹 더 막기 위해서입니다.
>
> 큐레이터는 탈퇴할 수 없습니다. 관리자가 사라지면 서비스가 멈추기 때문입니다.
...
> **📘 쉬운 설명**
>
> 알림을 받을 기기를 등록하는 창구입니다. 브라우저가 발급한 "이 기기로 알림 보내는 주소"를 서버에 맡깁니다.
>
> **부수효과** 부분이 세심합니다 — 알림 설정이 꺼져 있었다면 **자동으로 켭니다.** 사용자가 브라우저의 "알림 허용"을 눌렀다는 것 자체가 "받고 싶다"는 뜻인데, 여기서 또 스위치를 켜라고 하면 두 번 일하게 되고 대부분은 못 찾습니다.
...
> **📘 쉬운 설명**
>
> 큐레이터만 쓰는 창구들입니다. 전시 올리기, 회원 관리, 설정 변경.
>
> 맨 위 문장을 보세요 — 이 창구들은 **캐시하지 않고, 변경 작업은 전부 기록을 남깁니다.** 운영 화면에서 옛날 정보가 보이면 잘못된 판단을 하게 되고, 누가 뭘 바꿨는지 모르면 나중에 원인을 못 찾습니다.
...
> **📘 쉬운 설명**
>
> 관리자 홈의 날짜 목록입니다. 큐레이터가 이 화면에서 알고 싶은 건 딱 하나 — **"어느 날이 비어 있는가"**입니다.
>
> 마지막 문장이 중요합니다. 요청한 기간의 **모든 날짜를 빠짐없이** 돌려줍니다. 전시가 없는 날도 빈 줄로 포함시킵니다.
>
> 화면이 "8월 27일부터 9월 2일까지 7칸을 만들고, 받아온 3개를 맞는 자리에 끼워 넣기"를 하게 만들면 날짜 계산 실수가 생깁니다. 서버가 다 채워서 주면 화면은 받은 대로 그리기만 하면 됩니다.
...
> **📘 쉬운 설명**
>
> 전시 편집 화면에 필요한 모든 것입니다. 두 가지가 눈에 띕니다.
>
> **`slots`는 항상 12개입니다.** 그림이 3개만 올라가 있어도 빈 자리 9개를 포함해 12개를 돌려줍니다. 화면은 "3개 받았으니 9개는 빈칸을 만들어야지" 하고 계산할 필요 없이 받은 대로 그리면 됩니다.
>
> **`publish_blockers`(발행을 막고 있는 것들)** — "제목 없음", "5번 그림 미완성" 같은 목록입니다. 큐레이터가 "왜 아직 발행이 안 되지?"라고 헤매지 않도록 서버가 이유를 알려줍니다.
...
> **📘 쉬운 설명**
>
> 큐레이터가 글을 쓰는 동안 **자동으로 저장되는** 창구입니다. 타자를 멈추면 잠시 후 저절로 불립니다.
>
> `published_now`가 재미있는 항목입니다. **"이번 저장으로 발행 조건이 처음 충족되었다"**를 알려줍니다. 이 서비스에는 "발행" 버튼이 없고, 제목·테마·그림 12점이 다 차면 저절로 걸리기 때문에, 그 순간을 화면이 알아야 "전시가 발행되었습니다"를 띄울 수 있습니다.
...
> **📘 쉬운 설명**
>
> 사진을 올리는 방식이 조금 특이해서 설명이 필요합니다.
>
> 보통은 화면 → 서버 → 저장소 순서로 파일을 보냅니다. 그런데 이 서비스의 서버(Lambda)는 **6MB보다 큰 파일을 통과시키지 못합니다.** 사진은 20MB까지 허용해야 하니 그대로는 안 됩니다.
>
> 그래서 이렇게 합니다.
> 1. 화면이 서버에게 "사진 3장 올릴게요"라고 말한다
> 2. 서버가 **"이 주소로 직접 올리세요"라는 임시 주소**를 발급한다 (이게 Presigned URL)
> 3. 화면이 저장소로 **직접** 올린다 (서버를 안 거침)
>
> 짐을 사무실을 거치지 않고 창고로 바로 보내되, 사무실이 발급한 출입증을 붙여 보내는 것과 같습니다.
...
> **📘 쉬운 설명**
>
> 이 서비스에서 가장 인간적인 기능입니다.
>
> 큐레이터가 8월 31일 밤에 11점까지 올리고 설명을 쓰다가 자정을 넘겼다고 합시다. 규칙상 지난 날짜에는 새 전시를 걸 수 없습니다(백필 금지). 그렇다고 **하루치 노동을 통째로 버리게 하면**, 그 좌절이 곧 "이 서비스 그만할까"로 이어집니다. 기획서가 가장 큰 위험으로 꼽은 것이 바로 이것입니다.
>
> 그래서 **쓰다 만 원고를 오늘 날짜로 옮겨서 이어 쓸 수 있게** 했습니다. 막는 것은 "과거 날짜에 새 전시가 생기는 것"이지 "어제 쓴 글을 오늘 쓰는 것"이 아닙니다.
>
> 복사가 아니라 **이동**인 점도 중요합니다. 복사면 같은 원고가 두 군데 남아 헷갈립니다.
...
> **📘 쉬운 설명**
>
> 운영 설정을 읽고 바꾸는 창구입니다. **가입 잠금**(새 회원 받기 on/off)도 여기 있습니다.
>
> 마지막 문단이 설계 판단입니다. 기획서에는 "가입 잠금 전용 창구"가 있었는데, 조정할 설정이 10가지쯤 되고 화면은 하나뿐이라 **설정 전체를 다루는 창구 하나로 합쳤습니다.** 설정이 하나 늘 때마다 창구를 하나씩 만들면 금방 지저분해집니다.
...
> **📘 쉬운 설명**
>
> **헬스 체크**는 "너 살아 있니?"를 묻는 창구입니다. 감시 도구가 주기적으로 찔러보고, 답이 없으면 담당자에게 경보를 보냅니다.
>
> 아래 마지막 문장이 실무에서 중요한 함정입니다. 문제가 있어도 **`200`(정상)으로 답합니다.** 여기서 오류 코드를 내보내면 앞단(CloudFront)이 "서버가 죽었다"고 판단해서, **아무 문제 없던 첫 화면까지 안 뜨게** 만들어버리기 때문입니다.
...
> **📘 쉬운 설명**
>
> 지금까지 창구를 하나씩 봤다면, 여기서는 **실제 상황에서 어떤 순서로 불리는지**를 봅니다.
>
> 개별 창구는 다 이해했는데 "그래서 앱을 켜면 뭐부터 부르지?"를 모르는 상태가 흔합니다. 이 절이 그 답입니다. 화면 만드는 사람이 가장 먼저 보게 되는 부분이기도 합니다.
...
> **📘 쉬운 설명**
>
> 기획서에 스케치되어 있던 API 목록과 **달라진 부분, 그리고 그 이유**입니다.
>
> 기획서의 API 목록은 "이런 게 필요하겠다" 수준의 초안이었고, 실제로 설계하면서 합치거나 나누거나 새로 만든 것들이 생겼습니다. 각각에 이유를 붙여두면, 나중에 "왜 기획서랑 다르지?"라는 질문에 문서가 스스로 답합니다.
